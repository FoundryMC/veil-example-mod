...
public class MirrorBlockEntityRenderer implements BlockEntityRenderer<MirrorBlockEntity>, NativeResource {
    ...
    public static void renderLevel(ClientLevel level, Matrix4fc projection, DeltaTracker deltaTracker, CullFrustum frustum, Camera camera) {
        ...
        for (BlockPos pos : RENDER_POSITIONS) {
            ...
            VeilLevelPerspectiveRenderer.render(fbo, RENDER_MODELVIEW, RENDER_PROJECTION, renderPos, VIEW.identity().lookAlong(dir, up), RENDER_DISTANCE, deltaTracker, false);
            mirror.copy(fbo);
